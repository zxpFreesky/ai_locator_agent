...
SmartExecutor 是整个 ai_locator_agent 的核心，实现四级降级定位策略：
...
class SmartExecutor:
    ...
    async def smart_action(self, instruction: str) -> Dict[str, Any]:
        """
        多步任务规划与执行

        将复杂自然语言指令分解为原子操作序列，逐步执行。
        每步失败后可触发重规划。

        Args:
            instruction: 复杂自然语言指令（如"搜索耳机并点击第一个结果"）

        Returns:
            {
                "success": bool,
                "steps": [{"action": str, "instruction": str, "result": dict}, ...],
                "total_steps": int,
                "completed_steps": int,
                "error": str (仅在失败时)
            }
        ...
        steps = await self.task_planner.plan(instruction, dom or [])
        total = len(steps)
        if total == 0:
            return {"success": False, "error": "无法分解任务", "steps": [], "total_steps": 0, "completed_steps": 0}

        results = []
        action_map = {
            "click": self.smart_click,
            "fill": self.smart_fill,
            "select": self.smart_select,
            "hover": self.smart_hover,
            "check": self.smart_check,
        }
        ...
        for i, step in enumerate(steps):
            ...
            debug_print(f"[SmartAction] 步骤 {i+1}/{total}: {action} → {instr}" + (f" (值: {value})" if value else ""))

            handler = action_map.get(action)
            if not handler:
                step_result = {"success": False, "error": f"不支持的操作类型: {action}"}
            elif action == "fill":
                step_result = await handler(instr, value or "")
            elif action == "select":
                step_result = await handler(instr, value or "")
            else:
                step_result = await handler(instr)
            ...
            if not step_result.get("success"):
                debug_print(f"[SmartAction] 步骤 {i+1} 失败: {step_result.get('error', '未知错误')}")
                if i < total - 1:
                    await asyncio.sleep(1)
                    await self._wait_page_stable()
                    new_dom = await self.extractor.get_compact_dom(self.page, hint=instruction)
                    remaining = await self.task_planner.replan_remaining(
                        instruction, [s for s in results], step, new_dom or []
                    )
                    if remaining and len(remaining) > 0:
                        debug_print(f"[TaskPlanner] 重规划补充 {len(remaining)} 个步骤")
                ...
                return {
                    ...
                    "total_steps": total,
                    "completed_steps": i,
                    "error": f"步骤 {i+1} ({action}: {instr}) 失败: {step_result.get('error', '')}",
                }
        ...
        debug_print(f"[SmartAction] 全部 {total} 个步骤执行完成")
        return {
            "success": True,
            "steps": results,
            "total_steps": total,
