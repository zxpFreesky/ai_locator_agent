...
AI Locator Agent 的 MCP (Model Context Protocol) 服务端入口，
注册 10 个工具（navigate/smart_click/smart_fill/smart_select/smart_hover/
smart_check/smart_action/get_page_structure/screenshot/close_browser），
...
使用方式：
    python -m ai_locator_agent.server
    或直接: python ai_locator_agent/server.py
"""
...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
        ...
        types.Tool(name="smart_check", description="智能勾选复选框",
                   inputSchema={"type": "object", "properties": {"instruction": {"type": "string"}}, "required": ["instruction"]}),
        types.Tool(name="smart_action", description="智能执行多步操作任务（支持自然语言描述复杂操作流程，如\"\u641c索耳机并点击第一个结果\"\uff09",
                   inputSchema={"type": "object", "properties": {"instruction": {"type": "string"}}, "required": ["instruction"]}),
...
async def handle_call_tool(name: str, arguments: dict | None) -> list[types.TextContent]:
    ...
    try:
        ...
        if name in ("navigate", "smart_click", "smart_fill", "smart_select", "smart_hover", "smart_check", "smart_action", "get_page_structure", "screenshot"):
        ...
        if name == "navigate":
            ...
            result = await _executor.smart_action(arguments.get("instruction"))
            return [types.TextContent(type="text", text=json.dumps(result, ensure_ascii=False))]
