...
基于 Playwright 的 AI 驱动智能元素定位 MCP 服务，支持四级降级策略和多步任务规划，实现自然语言指令到精准元素操作的自动转换。
...
复杂指令 → TaskPlanner → 原子步骤分解 → 逐步执行（每步复用四级降级）
```

### 核心特性

- **四级降级策略**：从零消耗的本地 OCR 逐步降级到 LLM 坐标定位，确保高成功率
- **多步任务规划**：LLM 自动将复杂指令分解为原子操作序列，支持失败重规划
...
├── task_planner.py          # 多步任务规划器（LLM 分解复杂指令）
...
```python
import asyncio
from ai_locator_agent.server import ensure_browser, _page, _executor
...
```python
# 复杂指令自动分解为原子步骤
result = await executor.smart_action("在搜索框中输入耳机并点击搜索按钮")
# LLM 自动分解为:
#   1. fill("搜索输入框", "耳机")
#   2. click("搜索按钮")
# 每步复用四级降级策略，失败后自动重规划
...
| `smart_check` | 智能勾选复选框 |
| `smart_action` | 智能执行多步操作任务（自然语言描述复杂操作流程） |
...
TaskPlanner 将复杂自然语言指令分解为原子操作序列：
1. 采集当前页面 DOM 快照
2. LLM 生成结构化步骤列表（最多 10 步）
3. 逐步调用四级降级策略执行每个原子操作
4. 某步失败时，基于最新页面状态重新规划后续步骤
5. LLM 无法分解时，降级为单步 click 操作
